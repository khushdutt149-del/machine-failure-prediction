...
footfall = st.number_input("Footfall(The number of people or objects passing by the machine)", min_value=0)
tempMode = st.number_input("Temp Mode", min_value=0)
AQ = st.number_input("Air Quality (AQ)", min_value=0)
USS = st.number_input("Ultrasonic Sensor Data(USS)", min_value=0)
CS = st.number_input("Current Sensor (CS)", min_value=0)
VOC = st.number_input("VOC(Volatile Organic Compounds)", min_value=0)
RP = st.number_input("RP(Rotational position or RPM (revolutions per minute) of the machine parts.)", min_value=0)
IP = st.number_input("IP(Input pressure)", min_value=0)
